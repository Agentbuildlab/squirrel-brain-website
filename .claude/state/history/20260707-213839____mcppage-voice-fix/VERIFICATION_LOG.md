...
## 2026-07-07 — mcppage-voice-fix (commit 7a95c43)
- Render: OpenAI tts-1 / nova / mp3 → 64kbps 24kHz mono, 15.288s (original 15.317s). ffmpeg decode: 0 errors.
- Whisper round-trip: spoken words match new transcript verbatim ("Clawed code" = whisper homophone of "Claude Code").
- Gates: npx tsc --noEmit = 0 (×3 runs); npm run build (prod) = 0.
- Live drive: /mcp loads scuttle-mcppage.mp3 (206 range), playback advanced 9.8s/15.29s with error=null, on-call caption shows "Claude Code, Cursor, your own scripts".
- code-guardian (opus): PASS — no erasure (comment-only deletions), transcript byte-diff exactly ", ChatGPT"→" Code, Cursor", 19-unique-voices invariant restored, docs/SITE_SPEC.md correctly excluded from commit.
